  ...
  useEffect(() => {
    const fetchJugadorData = async () => {
      try {
        ...
        console.log(`/jugador/usuario/${userId}`);
        ...
        try {
          const perfil = await api.get(`/jugador/perfil/${jugadorId}`);
          console.log("Perfil obtenido:", perfil.data);

          const estadisticas = await api.get(
            `/api/jugador/estadisticas/${jugadorId}`,
          );
          console.log("Estadísticas obtenidas:", estadisticas.data);

          const entrenamientos = await api.get(
            `/api/jugador/entrenamientos/${jugadorId}`,
          );
