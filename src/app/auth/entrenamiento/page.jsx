...
const EntrenamientoForm = () => {
  ...
  const registrarDatosPosicion = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/posicion/registrar/${equipoId}`, {
        method: "POST",
  ...
  const generarEntrenamiento = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/entrenamientos/generar/${datoSesionId}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      });
  ...
  return (
    <div className="flex justify-center items-center bg-gray-100 w-full">
      <div className="bg-white p-6 rounded-md w-96 shadow-lg">
        ...
        <button onClick={registrarDatosPosicion} className="w-full bg-green-600 text-white p-3 rounded-md font-bold mb-2">Registrar datos de posición</button>
