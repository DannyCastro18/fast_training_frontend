    ...
    const fetchEquipoData = async () => {
      try {
        ...
        if (response.data.jugadores) {
          console.log("Jugadores en equipo:", response.data.jugadores);
          setJugadores(response.data.jugadores);
        } else {
          const jugadoresResponse = await axios.get("http://localhost:5000/api/jugadores?equipoId=2");
          console.log("Jugadores obtenidos de API:", jugadoresResponse.data);
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      ...
      <div className="mt-6 flex gap-4">
        ...
        <aside className="bg-blue-600 text-white p-6 rounded-lg shadow-lg w-1/3 flex flex-col items-center">
          ...
          <h2 className="text-2xl font-semibold">{equipo ? equipo.nombre : "Cargando..."}</h2>
          ...
          <div className="grid grid-cols-3 gap-4">
            {jugadores.length > 0 ? (
              jugadores.map((player, index) => (
                <div key={player.id} className="bg-gray-100 p-4 rounded-lg text-center shadow-md">
                  <Image
                    src="/jugador.png"
                    alt={`${player.usuarios?.persona?.nombre || "Jugador"} ${player.usuarios?.persona?.apellido || ""}`}
                    width={100}
                    height={100}
                    className="mx-auto rounded-full"
                  />
                  <p className="mt-2 text-gray-600 font-semibold text-lg">
                    {index + 1} {player.usuarios?.persona?.nombre} {player.usuarios?.persona?.apellido}
                  </p>
                  <p className="text-gray-600">{player.posicion}</p>
                </div>
              ))
            ) : (
              <p className="text-center col-span-3">No hay jugadores disponibles</p>
            )}
          </div>
        </main>
      </div>
