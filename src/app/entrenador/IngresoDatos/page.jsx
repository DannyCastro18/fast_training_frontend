...
export default function Page() {
  ...
  return (
    <div className="flex flex-col text-black ml-40 p-6 bg-gray-100 shadow-lg rounded-lg w-3/4 overflow-hidden">
      <h2 className="text-4xl font-bold text-left mb-4">Ingreso de datos de jugador</h2>
      <select 
      ...
      <div className="grid grid-cols-2 gap-6 w-full mt-6 overflow-hidden">
        {Object.keys(formData).map((key) => (
          key === "posicion" ? (
            ...
            <div key={key} className="flex flex-col  ">
              <label className="font-medium capitalize ">{key.replace(/_/g, ' ')}  {unidades[key] && <span className="text-gray-600">{unidades[key]}</span>}:</label>
              <input 
      ...
      <div className="flex justify-start mt-8 space-x-8">
        <button className="bg-gray-300 text-gray-600 p-2 rounded cursor-not-allowed ">Cancelar</button>
        <button onClick={handleSave} className="bg-blue-900 text-white p-2 rounded hover:bg-blue-700 transition">Guardar</button>
      </div>
    </div>
  );
}  